...
function cgi_get_POST_vars()
{
    ...
    elif [ "X${CONTENT_TYPE}" = "Xapplication/x-www-form-urlencoded" ]; then
...
if [ -z "$days" ]; then
    days=${SIMPLECA_EXPIRATION}
fi

cat | sed 's/\n/\r\n/g' > ${SIMPLECA_DIR}/web/${name}.csr
openssl ca -batch -notext -config ${SIMPLECA_DIR}/ca.conf -days ${days} -keyfile ${SIMPLECA_DIR}/ca/ca.key -cert ${SIMPLECA_DIR}/ca/ca.crt -passin file:${SIMPLECA_DIR}/ca/ca.passwd -in ${SIMPLECA_DIR}/web/${name}.csr -out ${SIMPLECA_DIR}/web/${name}.crt > /tmp/out.$$ 2>&1

if [ $? -ne 0 ]; then
    if [ -f ${SIMPLECA_DIR}/web/${name}.crt ]; then
        echo "Content-type: application/x-x509-ca-cert"
        echo
        cat ${SIMPLECA_DIR}/web/${name}.crt
    else
	    echo "Status: 500"
        echo "Content-type: text/plain"
        echo
